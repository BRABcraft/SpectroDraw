...
function drawSpriteOutline(useDelta,cx,cy){
  overlayCanvasPaint({cx,cy,useDelta});
}
function drawSampleRegion(cx) {
  overlayCanvasPaint({cx});
}
...
function drawPixel(xFrame, yDisplay, mag, phase, pan, bo, po, ch) {
  ...
  function processBin(bin, boScaled,opts={}) {
    ...
    //console.log(xFrame,visited,idx,layers[ch].mags.length);
    ...
    const [r, g, b] = magPhasePanToRGB(clampedMag, layers[ch].phases[idx], layers[ch].pans[idx]/2+0.5);
