function updateBrushPreview() {
  ...
  if (currentShape === "line") {
    ...
    pctx.shadowBlur = softness*10+blurRadius*((currentTool === "blur")?5:0);
    pctx.shadowOffsetX = 300;
    pctx.beginPath();
    pctx.moveTo(centerX - 320 , centerY + 20);
    pctx.lineTo(centerX - 280, centerY - 20);
    pctx.stroke();
    pctx.restore();
    ...
    pctx.beginPath();
    pctx.shadowColor = color;
    pctx.shadowBlur = softness*10+blurRadius*5;
    pctx.shadowOffsetX = 300;
    if (currentShape === "rectangle" || currentShape === "image") {
      pctx.save(); // Save current context state
      pctx.fillRect(centerX -300, centerY - 30, 60, 60);
      pctx.restore();
    } else {
      pctx.arc(centerX-300, centerY, radius*1.5 - 1, 0, Math.PI * 2);
      pctx.fill();
    }
    ...
    if (currentShape === "image"){
      ...
      gradient.addColorStop(1-softness, color);
